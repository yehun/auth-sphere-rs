...
pub async fn login_with_2fa(
    ...
    debug!("Platform 2FA login request: {:?}", req);
    ...
    let mfa_key = crate::server::service::generate_mfa_key(&UserKind::Platform, temp_token);
    ...
    match state.mfa_service.check(user_id, &username, otp_code).await {
        Ok(true) => {
            ...
            let user = state.user_service.get_user(UserKind::Platform, user_id).await.unwrap();
