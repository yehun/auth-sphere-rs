...
use auth_sphere_db::table::user::{UserId, UserKind};
...
pub async fn login_with_password(
    ...
    debug!("Member login request: {:?}", req);
...
pub async fn login_with_2fa(
    ...
    debug!("Member 2FA login request: {:?}", req);
    ...
    let mfa_key = crate::server::service::generate_mfa_key(&UserKind::Member, temp_token);
    ...
    match state.mfa_service.check(user_id, &username, otp_code).await {
        Ok(true) => {
            ...
            let user  = state.user_service.get_user(UserKind::Member, user_id).await.unwrap();
...
pub async fn register(
    ...
    debug!("Member register request: {:?}", req);
    ...
    match state.auth_service.register(
        ...
        Err(e) => {
            error!("Member registration failed: {}", e);
...
pub async fn logout(
    ...
    match state.auth_service.logout(UserKind::Member, token).await {
